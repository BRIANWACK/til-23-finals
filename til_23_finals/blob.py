...
class Blob():
    def __init__(self, bound, frame, dilate_iter=0, erode_iter=0, filter_by_size=None, heightMin=0, widthMin=0):
        self.frame = frame
        self.bound = bound
...
class ParticleFilter():
    def __init__(self, frame, cnts, P, N):
        self.frame = frame
        self.cnts = sorted(cnts, key=lambda x: cv2.contourArea(x)) # contours, sorted by size
        ...
        self.boundaryCnt = [np.array([[[0, 0]], [[w-1, 0]], [[w-1, h-1]], [[0, h-1]]])]
        
        # Generate frame where all pixels inside boundary and obstacles are 1.
        self.wallsAndBoundary = cv2.drawContours(self.walls.copy(), self.boundaryCnt, -1, 1, 1)
    ...
    def getDistGrid(self, gridCoord, angle):
        ...
        for xi in range(xP):
            for yi in range(yP):
                xCoord, yCoord = round(xMin+xi/xP*gridW), round(yMin-yi/yP*gridH)
                # print(xCoord, yCoord)
...
def plotDistGrid(distsGrids, deg, export=True):
    ...
    if export:
        plt.savefig(f"Presentation Images/Distance Grids/{deg}.svg", transparent=True, bbox="tight")
    plt.show()
...
def sensorFuseTest(deg, distsAll, sensors, alpha):
    ...
    intersectCoords = np.where(np.logical_and(np.logical_and(frontCond, leftCond), rightCond))
